...
export class VidsrcService {
  ...
  getIMDBShow(imdbID: string, season: number = 1, episode: number = 1, autonext: number = 0): Observable<string> {
    return this.http.get<any>(`${this._IMDB_SHOW_ENDPOINT}${imdbID}&season=${season}&episode=${episode}${autonext === 1 ? '&autonext=1' : ''}`, this._httpOptions).pipe(
      map(x => x.body)
    )
  }
  ...
  getTMDBShow(tmdbID: string, season: number, episode: number, autonext: number = 0): Observable<string> {
    return this.http.get<any>(`${this._TMDB_SHOW_ENDPOINT}${tmdbID}&season=${season}&episode=${episode}${autonext === 1 ? '&autonext=1' : ''}`, this._httpOptions).pipe(
      map(x => x.body)
    )
  }
}